const Tiles = ({ canPlay, setValue, board, winner }: { canPlay: { current: boolean }; setValue: (arg: any) => any; board: string[]; winner: number[] }) => {
  return (
    <div className={(!canPlay.current ? 'no ' : '') + "grid grid-cols-3 grid-rows-3 gap-2 p-3 w-[80vw] h-[80vw] max-h-[36rem] max-w-xl mx-auto bg-blue-600 rounded-3xl max-md:mt-[25vw]"} id="board">{
